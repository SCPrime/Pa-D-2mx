"""
MOD SQUAD Framework - Dependency Tracker Extension
...
TRACKER_LOG = (
    CONFIG_PATH.parent.parent / "logs" / "run-history" / "dependency_tracker.jsonl"
)
...
def register_dependency(consumer: str, provider: str, contract: str) -> None:
    ...
    # Avoid duplicates
    if not any(
        d["provider"] == provider and d["contract"] == contract for d in graph[consumer]
    ):
...
def alert_contract_change(
    provider: str, contract: str, change_description: str
) -> None:
...
__all__ = [
    "alert_contract_change",
    "get_dependents",
    "init_paiid_dependencies",
    "register_dependency",
]
