...
// Helper: Check for console errors (currently unused - for future debugging)
...
// Helper: Check for failed network requests (currently unused - for future debugging)
...
test.describe("PaiiD Radial Menu Wedge Tests", () => {
  ...
  test("1. Morning Routine - Live Data", async ({ page }) => {
    ...
    const accountRequest = page.waitForResponse(
      (response) =>
        response.url().includes("/api/account") || response.url().includes("/api/portfolio"),
      { timeout: 15000 }
    );
  ...
  test("4. Active Positions - Live Data", async ({ page }) => {
    ...
    const positionsRequest = page.waitForResponse(
      (response) =>
        ...
        response.url().includes("/api/proxy/positions"),
      { timeout: 15000 }
    );
  ...
  test("7. Backtesting - Live Data", async ({ page }) => {
    ...
    const backtestUI = await page
      .locator("text=/backtest|historical|test strategy|performance/i")
      .count();
  ...
  test("8. Execute Trade - Live Data", async ({ page }) => {
    ...
    const formElements = await page
      .locator("text=/symbol|quantity|order|buy|sell|market|limit/i")
      .count();
  ...
  test("9. Options Trading - Live Data", async ({ page }) => {
    ...
    try {
      ...
      const optionsContent = await page
        .locator("text=/option|strike|call|put|greek|chain/i")
        .count();
  ...
  test("11. ML Intelligence - Live Data", async ({ page }) => {
    ...
    const mlContent = await page
      .locator("text=/machine learning|ml|sentiment|pattern|prediction|intelligence/i")
      .count();
