...
class Settings(BaseModel):
    ...
    JWT_SECRET_KEY: str = Field(
        default_factory=lambda: os.getenv(
            ...
            else "test-secret-key-do-not-use-in-production",
        ),
    ...
    USE_TEST_FIXTURES: bool = Field(
        default_factory=lambda: os.getenv("USE_TEST_FIXTURES", "false").lower()
        == "true",
    ...
    def validate_api_token(cls, v: str) -> str:
        ...
        if len(v) < 20:
            raise ValueError(
                "API_TOKEN must be at least 20 characters long for security"
            )
        return v
